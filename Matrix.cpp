...
const uint8_t Matrix::MATRIX_FORWARD_ELIMINATION  = 0;
const uint8_t Matrix::MATRIX_BACKWARD_ELIMINATION = 1;
...
int Matrix::computeRank(int M, int Q, BitSequence **matrix)
{
    int	i, rank;
    int m = (M > Q) ? Q : M;
...
void Matrix::perform_elementary_row_operations(int flag, int i, int M, int Q, BitSequence **A)
{
    int j, k;
...
int Matrix::find_unit_element_and_swap(int flag, int i, int M, int Q, BitSequence **A)
{
    int index, row_op = 0;
...
int Matrix::swap_rows(int i, int index, int Q, BitSequence **A)
{
    int p;
    BitSequence temp;
...
int Matrix::determine_rank(int m, int M, int Q, BitSequence **A)
{
...
Matrix::BitSequence** Matrix::create_matrix(int M, int Q)
{
    int i;
    BitSequence **matrix;
...
void Matrix::def_matrix(int M, int Q, BitSequence **m,int k, AbstractTest *dataSrc)
{
    int i,j;
    for ( i=0; i<M; i++ )
        for ( j=0; j<Q; j++ )
            m[i][j] = dataSrc->getBit(k*(M*Q)+j+i*M);
}

void Matrix::delete_matrix(int M, BitSequence **matrix)
{
    int i;
