...
RandomExcursionsTest::RandomExcursionsTest() :
    AbstractTest(),
    testOk(false)
{
}
...
bool RandomExcursionsTest::testWasApplicable()
{
    return testOk;
}

double RandomExcursionsTest::runTest()
{
/* original code:
void
 ...
 */
    ...
    int		stateX[8] = { -4, -3, -2, -1, 1, 2, 3, 4 };
    int		counter[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
    double	p_value, sum, constraint, nu[6][8], min_p_value;
    double	pi[5][6] = { {0.0000000000, 0.00000000000, 0.00000000000, 0.00000000000, 0.00000000000, 0.0000000000},
                             {0.5000000000, 0.25000000000, 0.12500000000, 0.06250000000, 0.03125000000, 0.0312500000},
                             {0.7500000000, 0.06250000000, 0.04687500000, 0.03515625000, 0.02636718750, 0.0791015625},
                             {0.8333333333, 0.02777777778, 0.02314814815, 0.01929012346, 0.01607510288, 0.0803755143},
    ...
    if ( ((S_k = (int *)calloc(nob, sizeof(int))) == NULL) ||
         ((cycle = (int *)calloc(Tools::MAX(1000, nob/100), sizeof(int))) == NULL) )
    {
        fprintf(stderr,"RandomExcursionsTest::runTest(): "
    ...
    constraint = Tools::MAX(0.005*pow(nob, 0.5), 500.0);
    min_p_value = 0.0;
    if (J < constraint) {
        fprintf(stderr,"RandomExcursionsTest::runTest(): "
        ...
        for ( i=0; i<8; i++ ) {
            ...
            for ( k=0; k<6; k++ )
                    sum += pow(nu[k][i] - J*pi[(int)fabs(x)][k], 2) / (J*pi[(int)fabs(x)][k]);
            p_value = Cephes::cephes_igamc(2.5, sum/2.0);
            if (i == 0)
                min_p_value = p_value;
            else
                min_p_value = Tools::MIN(p_value,min_p_value);
        }
    }
    free(S_k);
    free(cycle);
