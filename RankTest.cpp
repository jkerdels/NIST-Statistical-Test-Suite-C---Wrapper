 ...
 */
    ...
    Matrix::BitSequence **matrix = Matrix::create_matrix(32, 32);
    ...
    product = 1.0;
    for (i = 0; i <= r-1; ++i)
        product *= ((1.e0-pow(2.0, i-32))*(1.e0-pow(2, i-32)))/(1.e0-pow(2, i-r));
    p_32 = pow(2, r*(32+32-r)-32*32) * product;
    ...
    for (k = 0; k < N; ++k) {
        Matrix::def_matrix(32, 32, matrix, k, this);

        R = Matrix::computeRank(32, 32, matrix);
        if ( R == 32 )
            F_32++;			// DETERMINE FREQUENCIES
        if ( R == 31 )
            F_31++;
    ...
    chi_squared =(pow(F_32 - (double)(N)*p_32, 2.0)/((double)(N)*p_32) +
                  pow(F_31 - (double)(N)*p_31, 2.0)/((double)(N)*p_31) +
                  pow(F_30 - (double)(N)*p_30, 2.0)/((double)(N)*p_30));
