...
double LongestRunOfOnesTest::runTest()
{
/* original code:
 ...
 */
    ...
    chi2 = 0.0;
    for (i = 0; i <= K; ++i){
        double td = (double)(nu[i]) - (double)(N) * pi[i];
        chi2 += (td * td) / ((double)(N) * pi[i]);
    }

    pval = Cephes::cephes_igamc((double)(K) / 2.0, chi2 / 2.0);
