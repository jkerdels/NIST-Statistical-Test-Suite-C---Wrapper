 ...
 */
    ...
    if (nob < 387840) {
        fprintf(stderr,"MaurersTest::runTest(): "
                       "Data should contain at least 387840 Bits!\n");
        return 0.0;
    }
    ...
    if ( nob >= 2068480 )    L = 8;
    if ( nob >= 4654080 )    L = 9;
    if ( nob >= 10342400 )   L = 10;
    if ( nob >= 22753280 )   L = 11;
    if ( nob >= 49643520 )   L = 12;
    if ( nob >= 107560960 )  L = 13;
    if ( nob >= 231669760 )  L = 14;
    if ( nob >= 496435200 )  L = 15;
    if ( nob >= 1059061760 ) L = 16;

    Q = 10*(int)pow(2, L);
    K = (int) (floor(nob/L) - (double)Q);	 		    // BLOCKS TO TEST
    ...
    for ( i=1; i<=Q; i++ ) {		// INITIALIZE TABLE
            decRep = 0;
            for ( j=0; j<L; j++ )
                    decRep += getBit((i-1)*L+j) * (long)pow(2, L-1-j);
            T[decRep] = i;
    }
    for ( i=Q+1; i<=Q+K; i++ ) { 	// PROCESS BLOCKS
            decRep = 0;
